...
import com.webex.connect.demo.CreateThreadActivity
import com.webex.connect.demo.R
import com.webex.connect.demo.data.InboxDataManager
import com.webex.connect.demo.databinding.FragmentMessagingBinding
import com.webex.connect.demo.util.CommonUtils

/**
 * Fragment for the messaging.
 */
class MessagingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        sharedViewModel.setHeaderTitle(getString(R.string.title_messaging))
        sharedViewModel.setConnStatusViewVisibility(true)
        ...
        adapter = InboxAdapter({ _, message ->
            ...
            if (threadId.isNotBlank()) {
                ...
                ConversationActivity.start(requireContext(), message.thread!!, true)
            } else {
        ...
        // Observe the conversation threads
        sharedViewModel.conversationThreads.observe(viewLifecycleOwner) { threads ->
        ...
        binding.fabCreateThread.setOnClickListener { view ->
            // Start the create thread activity
            CreateThreadActivity.start(view.context)
        }
